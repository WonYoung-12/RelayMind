		...
		try {
			while ((line = br.readLine()) != null) {
				...
				case CatchMindProtocol.GAME_START: {
					...
					String hint = question.setHint(answer);
					
					// 유저가 속해있는 방에 answer 바꿔주자.
					synchronized(userAl){
						for (int i = 0; i < roomAl.size(); i++) {
							if (roomAl.get(i).roomName.equals(roomName)) {
								roomAl.get(i).answer = answer;roomAl.get(i).hint = hint;
								System.out.println("해당방의 answer : " + roomAl.get(i).answer);
								System.out.println("해당방의 hint : " + roomAl.get(i).hint);
							}
						}
					}
							...
							if (roomAl.get(i).roomName.equals(roomName)) {
								...
								synchronized (userAl) {
									for (int j = 0; j < userAl.size(); j++) {
										if (userAl.get(j).turn == 0) {
											PrintWriter pw = userAl.get(j).pw;
											pw.println(msg);
											pw.flush();
											sw.time = sw.time + 15;
										}
				...
				case CatchMindProtocol.ITEM_2: {
					...
					for (int i = 0; i < roomAl.size(); i++) {
						if (roomAl.get(i).roomName.equals(roomName)) {
							String answer = roomAl.get(i).answer;
							String hint = roomAl.get(i).hint;
							String msg = CatchMindProtocol.ITEM_2 + ";" + answer + ";" + hint + ";";
							synchronized (userAl) {
								for (int j = 0; j < userAl.size(); j++) {
									if(userAl.get(j).turn == 0){
										PrintWriter pw = userAl.get(j).pw;
